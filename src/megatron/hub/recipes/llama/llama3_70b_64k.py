...
def model_config(
    ...
    Args:
        tensor_parallelism (int): Degree of tensor model parallelism. Default optimized for 70B with 64k sequences.
        pipeline_parallelism (int): Degree of pipeline model parallelism. Default optimized for 70B with 64k sequences.
        pipeline_parallelism_dtype (Optional[torch.dtype]): Data type for pipeline parallelism. Default optimized for 70B with 64k sequences.
        virtual_pipeline_parallelism (Optional[int]): Size of virtual pipeline parallelism. Default optimized for 70B with 64k sequences.
        context_parallelism (int): Degree of context parallelism. Default optimized for 70B with 64k sequences.
...
def pretrain_config(
    ...
    mock: bool = False,
    # Model configuration - defaults optimized for 70B with 64k sequences
    tensor_parallelism: int = 8,
    pipeline_parallelism: int = 4,
    pipeline_parallelism_dtype: Optional[torch.dtype] = torch.bfloat16,
    virtual_pipeline_parallelism: Optional[int] = None,
    ...
    """
    Create a pre-training configuration for Llama3 70B model with 64k sequence length.

    This function inherits from llama3_70b.pretrain_config() and overrides specific parameters
    optimized for 64k sequence length training.
    ...
    Args:
        ...
        precision_config (str | MixedPrecisionConfig): Precision recipe for the model.

    Returns:
        ConfigContainer: Configuration for pre-training.

    Note:
        Sequence length is set to SEQUENCE_LENGTH_64K (65536) for extended sequence training.
        Default parallelism settings are optimized for 70B model with 64k sequences efficiently.
    """
    # Get base configuration from llama3_70b with 64k sequence length
    cfg = llama3_70b.pretrain_config(
        dir=dir,
        name=name,
        ...
        seq_length=SEQUENCE_LENGTH_64K,  # Override to 64k sequence length
        lr=lr,
        min_lr=min_lr,
