...
from megatron.bridge.recipes.llama.llama3_70b_64k import SEQUENCE_LENGTH_64K, model_config, pretrain_config
...
class TestModelConfig:
    ...
    def test_model_config_default_parameters_70b_64k_optimized(self):
        """Test model_config with default parameters optimized for 70B with 64k sequences."""
        config = model_config()

        assert isinstance(config, Llama3ModelProvider70B)
        # Verify 70B + 64k optimized defaults
        assert config.tensor_model_parallel_size == 8  # High for 70B model
        assert config.pipeline_model_parallel_size == 4  # Moderate for 64k sequences
        ...
        assert config.context_parallel_size == 8  # High for 64k sequences
        assert config.sequence_parallel is True  # Enabled for 70B + 64k
    ...
    def test_model_config_sequence_length_consistency(self):
        """Test that model_config always uses the 64k sequence length constant."""
        configs = [
            model_config(),
        ...
        for config in configs:
            assert config.seq_length == SEQUENCE_LENGTH_64K, "Model sequence length should always be 64k"
...
class TestPretrainConfig:
    ...
    def test_pretrain_config_default_parameters_70b_64k_optimized(self):
        """Test pretrain_config with default parameters optimized for 70B with 64k sequences."""
        ...
        # Check that model uses 70B + 64k optimized defaults
    ...
    def test_pretrain_config_64k_sequence_length_override(self):
        """Test that sequence length is always set to 64k."""
    ...
    def test_pretrain_config_with_custom_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            config = pretrain_config(dir=temp_dir, name="test_70b_64k_run")

            expected_run_dir = os.path.join(temp_dir, "test_70b_64k_run")
    ...
    @pytest.mark.parametrize(
        ...
        [
            (8, 4, 8, True),  # Default 70B + 64k optimized
            (4, 4, 4, True),  # Different parallelism distribution
            (8, 2, 8, True),  # Higher context parallelism
            (4, 2, 4, False),  # Lower parallelism
        ],
    )
    def test_pretrain_config_70b_64k_parallelism_combinations(
        self, tensor_parallelism, pipeline_parallelism, context_parallelism, sequence_parallelism
    ):
        """Test various parallelism combinations for 70B model with 64k sequences."""
        config = pretrain_config(
    ...
    def test_pretrain_config_mock_mode_with_64k_sequence(self):
        """Test pretrain_config in mock mode with 64k sequence length."""
        config = pretrain_config(mock=True)

        assert config.dataset.sequence_length == SEQUENCE_LENGTH_64K  # Still 64k in mock mode
    ...
    def test_pretrain_config_batch_size_combinations(self, global_batch_size, micro_batch_size):
        ...
        # Sequence length should still be 64k regardless of batch size
        assert config.dataset.sequence_length == SEQUENCE_LENGTH_64K
    ...
    def test_pretrain_config_virtual_pipeline_parallelism(self, virtual_pipeline_parallelism):
        ...
        # Sequence length should still be 64k
        assert config.dataset.sequence_length == SEQUENCE_LENGTH_64K
