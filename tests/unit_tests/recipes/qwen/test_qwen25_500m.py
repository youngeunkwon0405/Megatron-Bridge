...
from megatron.bridge.recipes.qwen.qwen25_500m import model_config, pretrain_config
...
class TestModelConfig:
    ...
    def test_model_config_default_parameters(self):
        ...
        assert isinstance(config, Qwen25ModelProvider500M)
        assert config.tensor_model_parallel_size == 1  # Default for 500M model
...
class TestPretrainConfig:
    ...
    def test_pretrain_config_default_parameters(self):
        ...
        assert isinstance(config.model, Qwen25ModelProvider500M)
        ...
        # Check model configuration (500M specific defaults)
        assert config.model.tensor_model_parallel_size == 1  # Default for 500M model
    ...
    def test_pretrain_config_scheduler_configuration(self):
        ...
        assert config.scheduler.lr_warmup_iters == 500  # default for qwen25_500m
