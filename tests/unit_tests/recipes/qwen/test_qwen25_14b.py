...
from megatron.bridge.recipes.qwen.qwen25_14b import model_config, pretrain_config
...
class TestModelConfig:
    ...
    def test_model_config_default_parameters(self):
        ...
        assert isinstance(config, Qwen25ModelProvider14B)
        assert config.tensor_model_parallel_size == 4  # Default for 14B model
...
class TestPretrainConfig:
    ...
    def test_pretrain_config_default_parameters(self):
        ...
        assert isinstance(config.model, Qwen25ModelProvider14B)
        ...
        # Check model configuration (14B specific defaults)
        assert config.model.tensor_model_parallel_size == 4  # Default for 14B model
        assert config.model.pipeline_model_parallel_size == 1  # No PP by default
        assert config.model.pipeline_dtype is None  # No pipeline dtype by default
    ...
    def test_pretrain_config_scheduler_configuration(self):
        ...
        assert config.scheduler.lr_warmup_iters == 500  # default for qwen25_14b
