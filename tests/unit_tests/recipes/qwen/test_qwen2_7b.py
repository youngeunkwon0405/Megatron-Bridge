...
from megatron.bridge.models.qwen import Qwen2ModelProvider7B
from megatron.bridge.recipes.qwen.qwen2_7b import model_config, pretrain_config
...
class TestModelConfig:
    ...
    def test_model_config_default_parameters(self):
        ...
        assert isinstance(config, Qwen2ModelProvider7B)
        assert config.tensor_model_parallel_size == 2  # Default for 7B model
...
class TestPretrainConfig:
    ...
    def test_pretrain_config_default_parameters(self):
        ...
        assert isinstance(config.model, Qwen2ModelProvider7B)
        ...
        # Check model configuration (7B specific defaults)
        assert config.model.tensor_model_parallel_size == 2  # Default for 7B model
    ...
    def test_pretrain_config_scheduler_configuration(self):
        ...
        assert config.scheduler.lr_warmup_iters == 500  # default for qwen2_7b
    ...
    def test_pretrain_config_custom_tensor_parallelism(self):
        """Test pretrain_config with custom tensor parallelism."""
        config = pretrain_config(tensor_parallelism=4)

        assert config.model.tensor_model_parallel_size == 4
        assert config.model.pipeline_model_parallel_size == 1  # default
        assert config.model.context_parallel_size == 1  # default

    def test_pretrain_config_7b_specific_defaults(self):
        """Test that 7B model has appropriate defaults for its size."""
        config = pretrain_config()

        # 7B model should default to tensor parallelism of 2 for efficiency
