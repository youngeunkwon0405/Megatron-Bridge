...
# Run generate_from_hf.py with tp=1 and pp=2
python -m torch.distributed.run --nproc_per_node=2 --nnodes=1 \
    ...
    --max_new_tokens 10 \
    --tp 1 \
    --pp 2 > "$TEST_OUTPUT_DIR/generation_output.txt" 2>&1
...
echo "SUCCESS: Text generation test with Pipeline Parallelism (pp=2) completed successfully"
